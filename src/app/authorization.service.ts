...
export class AuthorizationService {
  ...
  constructor(
    ...
    let authorizationServiceConfiguration: AuthorizationServiceConfiguration | null =
      null;
    ...
    if (environment.issuer_uri === window.localStorage.getItem(LS_ISSUER_URI)) {
      ...
      const tokenResponseJSON = JSON.parse(
        window.localStorage.getItem(LS_TOKEN_RESPONSE)!
      );
    ...
    this._serviceConfigs = new BehaviorSubject(
      authorizationServiceConfiguration
    );
    ...
    combineLatest([this._serviceConfigs, this._tokenResponses]).subscribe(
      ([configuration, token]: [
        ...
        if (configuration.userInfoEndpoint == null) {
          console.log(
            "userinfo cannot be emitted - userinfo endpoint not specified by metadata"
          );
          this._userInfos.next(null);
          return;
        }
  ...
  public authorize(): void {
    this._serviceConfigs
      ...
      .subscribe((configuration: AuthorizationServiceConfiguration) => {
        ...
        console.log(AuthorizationRequest.RESPONSE_TYPE_CODE);
